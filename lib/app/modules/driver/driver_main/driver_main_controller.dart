...
class DriverMainController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    print('=== DRIVER MAIN CONTROLLER INIT ===');
  ...
  Future<void> _initializeCacheServiceAndCheckLogin() async {
    try {
      ...
      print('Cache service initialized in DriverMainController');
  ...
  Future<void> _checkLoginStatus() async {
    try {
      print('=== CHECKING LOGIN STATUS IN DRIVER MAIN ===');

      // Check Firebase Auth first
      final currentUser = _firebaseAuth.currentUser;
      print('Firebase current user: ${currentUser?.email}');
      print('Firebase user verified: ${currentUser?.emailVerified}');
      ...
      if (cachedUser != null && cachedUser.role == 'driver') {
        print('Driver is logged in and verified, staying on DriverMainView');
        // User is properly logged in as driver, stay on this view
      } else if (cachedUser == null) {
        print('WARNING: No cached user data found, but Firebase user exists');
        print(
          'This might be a timing issue. Staying on DriverMainView for now.',
        );
        ...
        print('ERROR: User is logged in but not a driver');
        print('Cached user: ${cachedUser?.toJson()}');
        // User is logged in but not a driver, redirect to signin
        Get.offAllNamed('/signin');
      }
    } catch (e) {
      print('=== DRIVER MAIN LOGIN CHECK ERROR ===');
  ...
  @override
  void onClose() {
    print('=== DRIVER MAIN CONTROLLER CLOSED ===');
    super.onClose();
  }
}
